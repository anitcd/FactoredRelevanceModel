...
public class TRECQueryParser extends DefaultHandler {
    ...
    StandardQueryParser cityQueryParser;    
    ...
    public TRECQueryParser(String queryFilePath, Analyzer analyzer) throws SAXException {
       ...
       Analyzer kAnalyzer = new KeywordAnalyzer();
       cityQueryParser = new StandardQueryParser(kAnalyzer);
    }
...
//----------------------------------------------------
            ...
            query.qcity = buff.toString().trim();
            buff.setLength(0);
        }
        else if (qName.equalsIgnoreCase("lat")) {
            query.qlat = buff.toString().trim();
            buff.setLength(0);
        }
        else if (qName.equalsIgnoreCase("lng")) {
            query.qlng = buff.toString().trim();
            buff.setLength(0);
        }
//----------------------------------------------------        
...
//        // Ani...
//        if(queryFieldFlag == 3) { // i.e. parsing only QID
//            Query luceneQuery = queryParser.parse(trecQuery.qid, "qQID");
//            return luceneQuery;
//        }
